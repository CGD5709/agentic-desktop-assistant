...
#### 3. Task Cancellation & Stop Command (`stop` / `cancel`)
Dispatched by the frontend when the user clicks the Stop button (Square icon) or interrupts the active thought/speech loop. Triggers asynchronous cancellation (`task.cancel()`) of the active LangGraph execution graph and resets server status to `IDLE`.
```json
{
  "type": "stop"
}
...
The synthesized conversational text produced by the agent. Returns both `content` (full Markdown styling for visual rendering) and `speech_text` (cleaned phonetic phrasing for Text-to-Speech synthesis generated by [`agent/voice_cleaner.py`](agent/voice_cleaner.py)).
```json
{
  "type": "assistant_message",
  "content": "He abierto Visual Studio Code y creado la rama **feature/login** exitosamente, señor.",
  "speech_text": "He abierto Visual Studio Code y creado la rama feature/login exitosamente, señor."
}
```
...
```text
[INFO] [Server] Initializing Reasoning Engine and dependent services...
[INFO] [Server] LangGraph Engine and WebSockets ready on http://0.0.0.0:8000
```
